...
async function testNoBackendRuntime () {
  ...
  const autosaveStart = app.indexOf('async function encryptedAutosave')
  const autosaveEnd = app.indexOf('function renderEditor')
  assert.notEqual(autosaveStart, -1)
  assert.notEqual(autosaveEnd, -1)
  const autosaveBody = app.slice(autosaveStart, autosaveEnd)
  assert.equal(autosaveBody.includes('articleEventTemplate'), false)
  assert.equal(autosaveBody.includes('publishCurrent'), false)
  assert(html.includes('script type="module"'), true)
  assert(html.includes('fork-awesome/css/fork-awesome.min.css'), true)
  assert(html.includes('vendor/codemirror/codemirror.min.js'), true)
  assert(html.includes('vendor/markdown-it/markdown-it.min.js'), true)
  assert(html.includes('codimd-css/index.css'), true)
  ...
  assert(html.includes('id="studioSigninButton"'), true)
  assert(html.includes('id="autosaveStatusLabel"'), true)
  ...
  assert(html.includes('class="status-bar codimd-statusbar"'), true)
  assert(html.includes('id="ui-toc"'), true)
